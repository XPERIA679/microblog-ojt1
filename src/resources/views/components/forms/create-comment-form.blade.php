@php
    $postId = $postsMediumOrShare instanceof PostShare ? $postsMediumOrShare->id : $postsMediumOrShare['post']->id;
    $idField = $postsMediumOrShare instanceof PostShare ? 'post_share_id' : 'post_id';
@endphp
...
<form class="bg-mycream p-4 items-center" method="POST" action="{{ route('post.comment.create') }}">
    ...
    <button class="flex items-center py-2 px-4 rounded-lg text-sm hover:bg-mydark hover:text-mycream bg-mywhite text-mydark shadow-lg transition-all">
        Comment
        <x-svgs.post-icon />
    </button>
</form>
